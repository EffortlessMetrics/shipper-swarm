...
Use the GitHub connector for PR metadata, diffs, threads, and review submission; use local `git`/`gh` only for gaps such as exact merge-base computation or Actions logs. Invoke `$orchestrate-work` or explicit read-only explorer/reviewer skills when parallel lenses materially improve coverage. Workers must not edit, commit, push, resolve threads, or change the PR while the review is in progress. The coordinating Codex instance owns deduplication, the final inline review, and the candidate result.
