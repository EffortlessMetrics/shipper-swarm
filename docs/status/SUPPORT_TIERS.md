...
| Status JSON registry comparison | stable | `cargo test -p shipper-cli --test e2e_status status_json_format_produces_registry_report`; `cargo test -p shipper-cli --test e2e_status status_completed_outcome_has_human_json_parity_without_side_effect_claims`; `shipper status --format json` emits `shipper.status.v1` registry/package state plus a read-only typed outcome | cli/integrations |
