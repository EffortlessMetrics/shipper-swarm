...
| Resume JSON receipt output | stable | `cargo test -p shipper-cli --test bdd_resume given_pending_state_when_resume_json_then_stdout_is_receipt_json`; `shipper resume --format json` emits machine-readable receipt JSON on stdout for the targeted registry | cli/integrations |
