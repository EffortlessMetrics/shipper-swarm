...
| `crypto-proptests-heavy` | `push` / `workflow_dispatch` / `schedule` | ~60 min (90-minute budget) | Full-strength `proptest` for `shipper-encrypt` round-trips. **Not** on PR — too slow. |
